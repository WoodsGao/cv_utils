...
def split_coco_image(coco_path, img_root, img_size, steps):
    print(img_size)
    ...
    for img_info in coco['images']:
        ...
        dirname = osp.dirname(img_info['file_name'])
        dirname = osp.join(dirname, 'split')
        os.makedirs(osp.join(img_root, dirname), exist_ok=True)
        img_name = osp.splitext(osp.basename(img_info['file_name']))[0]
        img_name = osp.join(dirname, img_name)
        anns = [
        ...
        for si, (img, anns) in enumerate(zip(imgs_split, anns_split)):
            ...
            cv2.imwrite(osp.join(img_root, iname), img)
            print(osp.join(img_root, iname))
            img_info = {
                'file_name': iname,
                'width': img.shape[1],
                'height': img.shape[0],
            ...
            insert_img_anns(new_coco, img_info, anns)
    new_coco = sort_coco(new_coco)
    save_path = osp.splitext(osp.basename(coco_path))[0] + '_split.json'
...
if __name__ == "__main__":
    ...
    split_coco_image(opt.coco, opt.img_root, img_size, steps)
